...
class GenehearingAnalyser(TonalAudiometry):
        ...
        def create_dataframe_for_merging(self, output_path):
            ...
            self.new_df = merged_df.loc[:, [self.patient_number_columnname, self.date_column, 'IF_FIRST', 'L_BIAP', 'P_BIAP', 'L_HEARING_TYPE', 'P_HEARING_TYPE', 'PATID']]
        ...
        def create_disinct_datasets(self, hearing_loss_dict, output_path):
            for loss_type, rules in hearing_loss_dict.items():
                ...
                df_filtered = self.new_df[( ((self.new_df['L_BIAP']==rules[0]) | (self.new_df['P_BIAP']==rules[1])) | ((self.new_df['L_BIAP']==rules[1]) | (self.new_df['P_BIAP']==rules[0])) )]                
